function [qrs_amp_raw,qrs_i_raw,delay,ecg_m,qrs_i,qrs_c]=pan_tompkin(ecg,fs,gr)
%Vignesh Kalidas 
...
if length(ecg_m) < 2*fs % Length of the signal too small
    THR_SIG = max(ecg_m)*1/4; % 0.25 of the max amplitude 
THR_NOISE = mean(ecg_m)*1/2; % 0.5 of the mean signal is considered to be noise
else
...
if length(ecg_h) < 2*fs % If length of the signal is too small
 THR_SIG1 = max(ecg_h)*1/4; % 0.25 of the max amplitude 
THR_NOISE1 = mean(ecg_h)*1/2; %
else
